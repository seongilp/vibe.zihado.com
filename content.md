...
구글 타임라인/Takeout 위치 데이터를 서버 전송 없이 브라우저에서만 파싱해 지도로 보여주는 개인 위치 기록 뷰어.
- 4가지 타임라인 형식 내용 기반 자동 감지, Takeout ZIP 그대로 드래그해 IndexedDB에 저장
- **스택**: Next.js 16 · TypeScript · Leaflet · Dexie(IndexedDB) · fflate
- **🎓 배울 수 있는 것**: 여러 데이터 포맷 방어적 파싱 전략 / 브라우저만으로 대용량 개인 데이터 다루는 로컬 퍼스트 설계
